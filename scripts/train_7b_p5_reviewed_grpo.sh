...
P5_DATA_DIR="${P5_DATA_DIR:-${PROJECT_ROOT}/data/processed/p5_grpo}"

if [[ ! -f "${SFT_ADAPTER_PATH}/adapter_model.safetensors" ]]; then
  echo "Frozen Stage 8 SFT20 adapter not found: ${SFT_ADAPTER_PATH}" >&2
  exit 1
fi
if [[ ! -f "${P5_DATA_DIR}/train.parquet" || ! -f "${P5_DATA_DIR}/tune.parquet" ]]; then
  echo "Prepare human-reviewed P5 GRPO data first: python scripts/prepare_p5_grpo.py" >&2
  exit 1
fi
"${PROJECT_ROOT}/.venv/bin/python" "${PROJECT_ROOT}/scripts/verify_p5_training_inputs.py" \
  --data-dir "${P5_DATA_DIR}"
...
export TRAIN_DATA="${TRAIN_DATA:-${P5_DATA_DIR}/train.parquet}"
export VAL_DATA="${VAL_DATA:-${P5_DATA_DIR}/tune.parquet}"
export LORA_ADAPTER_PATH="${SFT_ADAPTER_PATH}"
export OUTPUT_DIR="${OUTPUT_DIR:-${PROJECT_ROOT}/checkpoints/p5_reviewed_grpo_7b}"
...
exec bash "${PROJECT_ROOT}/scripts/train_grpo_smoke.sh" \
  ...
  trainer.experiment_name="${WANDB_EXPERIMENT:-p5-unseen-db-reviewed-replay-grpo-7b}" \
  trainer.val_before_train=false \
  trainer.test_freq=-1 \
  actor_rollout_ref.model.lora.merge=false \
  actor_rollout_ref.model.use_fused_kernels=true \
  actor_rollout_ref.model.fused_kernel_options.impl_backend=triton \
  actor_rollout_ref.actor.optim.lr="${GRPO_LEARNING_RATE:-2e-7}" \
  actor_rollout_ref.actor.kl_loss_coef="${KL_COEF:-0.02}" \
  ...
  +reward.reward_kwargs.duplicate_penalty="${DUPLICATE_PENALTY:-0.10}" \
  +reward.reward_kwargs.repeated_tool_penalty="${REPEATED_TOOL_PENALTY:-0.10}" \
  +reward.reward_kwargs.turn_limit_penalty="${TURN_LIMIT_PENALTY:-0.6}" \
  +reward.reward_kwargs.missing_submit_penalty="${MISSING_SUBMIT_PENALTY:-0.6}" \
