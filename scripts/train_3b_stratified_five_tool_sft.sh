...
INITIAL_ADAPTER_PATH="${INITIAL_ADAPTER_PATH:-${PROJECT_ROOT}/checkpoints/stage3_five_tool_sft_3b_native_v2/global_step_160/merged/lora_adapter}"

if [[ ! -f "${INITIAL_ADAPTER_PATH}/adapter_model.safetensors" ]]; then
  echo "Initial Tool-SFT adapter not found: ${INITIAL_ADAPTER_PATH}" >&2
  exit 1
fi
...
export OUTPUT_DIR="${OUTPUT_DIR:-${PROJECT_ROOT}/checkpoints/stage5_stratified_five_tool_sft_3b}"
...
exec bash "${PROJECT_ROOT}/scripts/train_sft_smoke.sh" \
  trainer.experiment_name=stage5-stratified-five-tool-sft-3b \
