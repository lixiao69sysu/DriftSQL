...
# Conservative Stage-8 GRPO retry.  Always restart from the best Tune model
# (SFT20), because the first GRPO trial reduced deterministic Tune accuracy.
...
export OUTPUT_DIR="${OUTPUT_DIR:-${PROJECT_ROOT}/checkpoints/stage8_conservative_grpo_7b}"
...
exec bash "${PROJECT_ROOT}/scripts/train_grpo_smoke.sh" \
  ...
  trainer.experiment_name=stage8-fresh-db-conservative-grpo-7b \
