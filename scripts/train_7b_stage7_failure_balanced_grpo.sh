...
SFT_ADAPTER_PATH="${SFT_ADAPTER_PATH:-${PROJECT_ROOT}/checkpoints/stage7_add_column_sft_7b/global_step_20/merged/lora_adapter}"

if [[ ! -f "${SFT_ADAPTER_PATH}/adapter_model.safetensors" ]]; then
  echo "Stage 7 SFT adapter not found: ${SFT_ADAPTER_PATH}" >&2
  exit 1
fi
if [[ ! -f "${PROJECT_ROOT}/data/processed/stage7_failure_balanced_grpo/train.parquet" ]]; then
  echo "Build the Stage 7 failure-balanced data first." >&2
  exit 1
fi

# The user-authorized devices for Stage 7.  TP=2 is the layout already proven
# by the Stage 5 7B run; TP=1 would keep a full 14.8 GiB rollout replica beside
# every actor shard and has no backward-pass headroom on 24 GiB cards.
...
export TRAIN_DATA="${TRAIN_DATA:-${PROJECT_ROOT}/data/processed/stage7_failure_balanced_grpo/train.parquet}"
export VAL_DATA="${VAL_DATA:-${PROJECT_ROOT}/data/processed/stage7_add_column_sft/rl_tune.parquet}"
export LORA_ADAPTER_PATH="${SFT_ADAPTER_PATH}"
export OUTPUT_DIR="${OUTPUT_DIR:-${PROJECT_ROOT}/checkpoints/stage7_failure_balanced_grpo_7b}"
...
# Keep W&B state in the project and upload online during training.
...
exec bash "${PROJECT_ROOT}/scripts/train_grpo_smoke.sh" \
  ...
  trainer.experiment_name=stage7-add-column-failure-balanced-grpo-7b \
  ...
  +reward.reward_kwargs.turn_limit_penalty="${TURN_LIMIT_PENALTY:-0.35}" \
  +reward.reward_kwargs.missing_submit_penalty="${MISSING_SUBMIT_PENALTY:-0.3}" \
